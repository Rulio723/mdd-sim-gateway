...
**不在 `main` 上直接开发，也不从 `main` 开分支。** `feat/*` 和 `fix/*` 从 `develop` 分出、
合回 `develop`；`release/*` 从 `develop` 分出，完成后合入 `main` 并打 `vX.Y.Z` 标签，发布后
把 `main` 合回 `develop`。紧急修复走同一条路，只是尽快发布，没有绕过 `develop` 的捷径——
否则 `main` 就不再等于已发布的内容，而这正是它唯一要保证的事。

`main` 只接收发布合并。标签是发布动作本身：推 `vX.Y.Z` 才会构建并推送镜像。
